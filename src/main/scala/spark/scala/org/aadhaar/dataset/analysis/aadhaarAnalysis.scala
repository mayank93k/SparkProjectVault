...
object aadhaarAnalysis {
  ...
  case class Problem1(data: String, registrar: String, private_agency: String, state: String, district: String, sub_district: String, pincode: String, gender: String,
    ...
    val splitData = readData.map {
      line =>
        ...
        /**
          ...
          */
        splitData.registerTempTable("problem1")
        ...
        splitData.groupBy('state, 'private_agency).agg(count('state)).select('state, 'private_agency).orderBy(desc("state"),
                                                                                   desc("private_agency")).show(20, false)
        ...
        splitData.select('state, 'aadhaar_generated).groupBy('state).agg(count('aadhaar_generated).as("countOfAdharCards"))
                                                                                .orderBy(desc("countOfAdharCards")).show(3, false)
        ...
        splitData.filter(splitData("gender") === "M").groupBy('state).agg(count('aadhaar_generated).as("countOfAdharCards"))
                                                                                 .orderBy(desc("countOfAdharCards")).select('state).show(3, false)
        ...
        /**
          ...
          */
        splitData.filter(splitData("gender") === "M").groupBy('state).agg(count('aadhaar_generated).as("countOfAdharCards"))
                                        .orderBy(desc("countOfAdharCards")).select('state).limit(3).collect().map(x => {
